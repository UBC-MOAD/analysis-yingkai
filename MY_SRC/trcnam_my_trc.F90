MODULE trcnam_my_trc
   ...
   USE trcsms_my_trc   ! parameters defined there
...
CONTAINS
   ...
   SUBROUTINE trc_nam_my_trc
      ...
      NAMELIST/trc_source/   cn_dir, sn_Ba_river   , sn_d18O_river
      NAMELIST/trc_boundary/ cn_dir, sn_Ba_boundary, sn_d18O_boundary
      NAMELIST/trc_ini/      cn_dir, sn_Ba_ini     , sn_d18O_ini
      NAMELIST/trc_ice/      cn_dir, sn_ice
      NAMELIST/trc_rnf/      cn_dir, sn_rnf      
      CALL ctl_opn( numnatl, 'namelist_my_trc', 'OLD', 'FORMATTED', 'SEQUENTIAL', 1, numout, .FALSE. )

      REWIND( numnatl)
      READ  ( numnatl, trc_ini ) 
      REWIND( numnatl )
      READ  ( numnatl, trc_boundary )
      REWIND( numnatl )
      READ  ( numnatl, trc_source )
      REWIND( numnatl )
      READ  ( numnatl, trc_ice)
      REWIND( numnatl )
      READ  ( numnatl, trc_rnf)
      !if (lwp) write(numout, *) 'cn_dir', cn_dir
