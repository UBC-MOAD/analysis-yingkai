MODULE trcnxt
   ...
   USE trdtra
   USE tranxt
   USE trcsms_my_trc, ONLY: Ba_ini, d18O_boundary, Ba_boundary, d18O_ini
...
CONTAINS
   ...
   SUBROUTINE trc_nxt( kt )
      ...
      !
      INTEGER  ::   i, j, k, k2, jn, jk              ! dummy loop indices
...
! Assign boundary conditions
      DO i = 1, jpi
         DO j = 1, jpj
            DO k = 1, jpk
               IF (Ba_boundary(i, j, k) > 0.1) THEN
                  trn(i, j, k, jpmyt1) = Ba_boundary(i, j, k)
                  trn(i, j, k, jpmyt2) = d18O_boundary(i, j, k)
               ENDIF
            ENDDO
         ENDDO
      ENDDO
! Bottom open boundary for tracers
!      trn(:, :, 39:jpk, jpmyt1) = 0.
!      trn(:, :, 39:jpk, jpmyt2) = 0.
