...
CONTAINS
   ...
   SUBROUTINE fld_read( kt, kn_fsbc, sd, map, jit, time_offset )
      ...
      LOGICAL  ::   llnxtwk   ! open next week file?
      LOGICAL  ::   llnxtfday   ! open next five day file?
      LOGICAL  ::   llnxtday   ! open next daily file?
      ...
      ENDIF
!      IF(lwp) write (numout, *) 'isecsbc', isecsbc
      imf = SIZE( sd )
      !
      ...
      IF( MOD( kt-1, kn_fsbc ) == 0 ) THEN         ! update field at each kn_fsbc time-step !
         !                                         ! ====================================== !
         !
!byC      IF(lwp) write (numout, *) 'going into loop'
         DO jf = 1, imf                            ! ---   loop over field   --- !
!C         IF(lwp) write (numout, *) 'sd(jf)%nrec_a(2)', sd(jf)%nrec_a(2) 
...
!byC           IF(lwp) write (numout, *) 'colour', sd(jf)%ln_tint
               ...
               IF( sd(jf)%ln_tint ) THEN
                  ...
                  ELSE                                                                    ! higher frequency mean (in hours)
                     ...
                     ELSEIF( sd(jf)%cltype(1:4) == 'fday' )    THEN   ;   ireclast = 24 * 5                  / sd(jf)%nfreqh
                     ELSEIF( sd(jf)%cltype      == 'daily'   ) THEN   ;   ireclast = 24                      / sd(jf)%nfreqh
                     ELSE                                             ;   ireclast = 24 * nyear_len(     1 ) / sd(jf)%nfreqh
                     ENDIF
                  ENDIF
!                  IF(lwp) write (numout, *) ireclast, 'ireclast', sd(jf)%nrec_a(1), sd(jf)%nrec_b(1)
...
!                     IF(lwp) write (numout, *) nyear, nmonth, nday
                     ...
                     IF( .NOT. sd(jf)%ln_clim ) THEN   ! close the current file and open a new one.
                        isec_week = 0
                        isec_fday = 0
                        IF (sd(jf)%cltype == 'yearly') THEN
                           llnxtyr = .TRUE.
                        ELSEIF (sd(jf)%cltype == 'monthly') THEN
                           llnxtmth = .TRUE.
                           llnxtyr = llnxtmth .AND. nmonth == 12
                        ELSEIF (sd(jf)%cltype == 'week') THEN
                           llnxtwk = .TRUE.
                           isec_week = NINT(rday) * 7 - ksec_week( sd(jf)%cltype(6:8) )
                           llnxtmth = isec_week > nmonth_len(nmonth)*NINT(rday) - nsec_month
                           llnxtyr = llnxtmth .AND. nmonth == 12
                        ELSEIF (sd(jf)%cltype == 'fday') THEN
                           llnxtfday = .TRUE.
                           isec_fday = NINT(rday) * 5 - MOD( nsec_fday, 5*NINT(rday))
!                           IF(lwp) write (numout, *) isec_fday, nsec_fday, nsec_month
                           llnxtmth = isec_fday >=  nmonth_len(nmonth)*NINT(rday) - nsec_month
                           llnxtyr = llnxtmth .AND. nmonth == 12
                        ELSEIF (sd(jf)%cltype == 'daily') THEN
                           llnxtday = .TRUE.
                           llnxtmth = llnxtday .AND. nday == nmonth_len(nmonth)
                           llnxtyr = llnxtmth .AND. nmonth == 12
                        ELSE
                           IF(lwp) write (numout, *) 'you should not have seen this line, bad cltype'
                           stop
                        ENDIF

!                        IF(lwp) write (numout, *) llnxtday, nmonth_len(imonth-1), llnxtmth, isec_week, isec_fday, rday

                        iyear  = nyear  + COUNT((/llnxtyr /))
                        imonth = nmonth + COUNT((/llnxtmth/)) - 12 * COUNT((/llnxtyr /))
                        iday   = nday   + COUNT((/llnxtday/)) - nmonth_len(imonth-1) * COUNT((/llnxtmth/)) + CEILING((isec_week + isec_fday) / rday)
!                        IF(lwp) write (numout, *) iyear, nyear
!                        IF(lwp) write (numout, *) imonth, nmonth
                        IF(lwp) write (numout, *) 'test iday,nday',iday,nday,COUNT((/llnxtday/)),nmonth_len(imonth-1) * COUNT((/llnxtmth/)),CEILING((isec_week + isec_fday) / rday)
                        ...
                        CALL fld_clopn( sd(jf), iyear, imonth, iday, llstop )
...
!                     &                 sd(jf)%nrec_a(1), REAL(sd(jf)%nrec_b(2),wp)/rday, REAL(sd(jf)%nrec_a(2),wp)/rday
   ...
   SUBROUTINE fld_init( kn_fsbc, sdjf, map )
      ...
      LOGICAL :: llprevfday            ! are we reading previous 5 day file?
      ...
      INTEGER :: isec_fday             ! number of seconds since start of a five day file
      CHARACTER(LEN=1000) ::   clfmt   ! write format
      !!---------------------------------------------------------------------
      ...
      IF( (sdjf%cltype(1:4) == 'week' .OR. sdjf%cltype(1:4) == 'fday') .AND. nn_leapy == 0 )   &
         &   CALL ctl_stop('fld_clopn: weekly/5 day file ('//TRIM(sdjf%clrootname)//') needs nn_leapy = 1')
      IF( (sdjf%cltype(1:4) == 'week' .OR. sdjf%cltype(1:4) == 'fday') .AND. sdjf%ln_clim  )   &
         &   CALL ctl_stop('fld_clopn: weekly/ 5 day file ('//TRIM(sdjf%clrootname)//') needs ln_clim = .FALSE.')
      ...
      IF( sdjf%ln_tint ) THEN ! we need to read the previous record and we will put it in the current record structure
         ...
         IF( sdjf%nrec_a(1) == 0  ) THEN   ! we redefine record sdjf%nrec_a(1) with the last record of previous year file
            ...
            ELSE                                ! higher frequency mean (in hours)
               ...
               ELSEIF( sdjf%cltype      == 'fday' ) THEN   ! five day file
                  llprevfday = .TRUE.                                                      ! use previous 5 day  file?
                  sdjf%nrec_a(1) = 24 * 5 / sdjf%nfreqh                                    ! last record of previous 5 day
                  isec_fday = NINT(rday) * 5                                               ! add a shift toward previous 5 day
         ...
         ELSEIF ( sdjf%cltype(1:4) == 'fday' ) THEN
!C           IF(lwp) write (numout,*) "calculate", isec_fday, nsec_fday
            isec_fday = isec_fday + MOD( nsec_fday, 5*NINT(rday))
!            IF(lwp) write (numout, *) isec_fday, nsec_month
            llprevmth = isec_fday > nsec_month
            llprevyr  = llprevmth .AND. nmonth == 1
         ENDIF
         llprev = llprevyr .OR. llprevmth .OR. llprevweek .OR. llprevfday .OR. llprevday
         !
         iyear  = nyear  - COUNT((/llprevyr /))
         imonth = nmonth - COUNT((/llprevmth/)) + 12 * COUNT((/llprevyr /))
         iday   = nday   - COUNT((/llprevday/)) + nmonth_len(nmonth-1) * COUNT((/llprevmth/)) - FLOOR((isec_week + isec_fday) / rday)
         !
!         IF(lwp) write (numout,*) iyear, imonth,iday
         IF(lwp) write (numout, *) 'prev. test iday,nday',iday,nday,COUNT((/llprevday/)),nmonth_len(imonth-1) *COUNT((/llprevmth/)),FLOOR((isec_week + isec_fday) / rday)
...
!          IF(lwp) WRITE(numout, clfmt) TRIM(sdjf%clvar), sdjf%nrec_a(1), REAL(sdjf%nrec_a(2),wp)/rday
      ...
      IF( sdjf%num <= 0 ) THEN
         ...
         IF ( sdjf%cltype(1:4) == 'week' ) THEN
            ...
            llprevyr   = llprevmth .AND. nmonth == 1
         ELSEIF ( sdjf%cltype(1:4) == 'fday') THEN
!C          IF(lwp) write (numout,*) "calculate future", isec_fday, nsec_fday
            isec_fday = MOD( nsec_fday, 5*NINT(rday))
            llprevmth = isec_fday > nsec_month
            llprevyr = llprevmth .AND. nmonth == 1
         ELSE
            isec_week  = 0
         ...
         iyear  = nyear  - COUNT((/llprevyr /))
         imonth = nmonth - COUNT((/llprevmth/)) + 12 * COUNT((/llprevyr /))
         iday   = nday   + nmonth_len(nmonth-1) * COUNT((/llprevmth/)) - FLOOR((isec_week + isec_fday) / rday)
!         IF(lwp) write (numout, *) 'future', iyear, imonth, iday
   ...
   SUBROUTINE fld_rec( kn_fsbc, sdjf, ldbefore, jit, time_offset )
      ...
      INTEGER  ::   isec_fday   ! number of seconds since the start of the five day file 
      INTEGER  ::   itime_add   ! local time offset variable
      ...
      ELSE                                   ! higher frequency mean (in hours)
         ...
         IF( sdjf%cltype(1:4) == 'fday' )   isec_fday = MOD( nsec_fday, 5*NINT(rday))   ! since the first day of the current 5 day
         ...
         ELSEIF( sdjf%cltype(1:4) == 'fday'    ) THEN   ;   ztmp = REAL(isec_fday, wp)  ! since the first day of the current 5 day
         ...
         IF( sdjf%cltype(1:4) == 'fday'    )   iendrec = iendrec + ( nsec_year - isec_fday )
   ...
   SUBROUTINE fld_clopn( sdjf, kyear, kmonth, kday, ldstop )
      ...
      IF( sdjf%cltype == 'fday') &
                                         WRITE(sdjf%clname, '(a,"d", i2.2)' ) TRIM( sdjf%clname     ), kday    ! add day and note that files are named by last day not first
!byC  IF(lwp) write(numout,*) sdjf%clname, 'filename'
      !
      CALL iom_open( sdjf%clname, sdjf%num, ldstop = ldstop, ldiof =  LEN(TRIM(sdjf%wgtname)) > 0 )

!byC  IF(lwp) write(numout, *) 'finished with clopn'
     !
